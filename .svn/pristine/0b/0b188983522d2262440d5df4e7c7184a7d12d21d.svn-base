...
<script>
   ...
   function refreshData()
   {
      ...
      var postData = { dateSelected: selectedDate, selectedStore: selectedStore, cg: cg, user: user, license: license, token: token };
      ...
      if (len > 2) {
         ...
         for (var i = 0; i <= len; i++)
         {
            ...
            if (typeof (tbl[0].rows[i].dataset.storecg) !== 'undefined' && tbl[0].rows[i].cells[3].children[0].checked)
            {
               ...
               checked.cells[3].childNodes[0].setAttribute('checked', 'true');
         ...
         var text = $('#tbStoreName').val();

         var ret = searchStore(text).done(function (data)
         {
            ...
            if (data == null)
            {
               $('#igStore').addClass('has-error');
               $('#igStore').removeClass('has-success');
            }
            else
            {
               $('#igStore').addClass('has-success');
               $('#igStore').removeClass('has-error');

               for (var i = 0; i < data.length; i++)
               {
                  // need to check for duplicate rows from above
                  var exists = skip.indexOf(data[i].StoreName);

                  if (exists == -1)  // if we have a duplicate store name, skip
                  {
                     html += "<tr data-storeName=\"" + data[i].StoreName + "\" data-storeCg=\"" + data[i].CG + "\" data-storeSerial=\"" + data[i].StoreSSN + "\" data-storeCount=\"" + data[i].CG_COUNT + "\">";
                     html += "<th class=\"text-left\">" + data[i].StoreName + "</th>";
                     html += "<th class=\"text-center\">" + data[i].CG + "</th>";
                     html += "<th class=\"text-center\">" + data[i].StoreSSN + "</th>";
